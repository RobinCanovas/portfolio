...
export function Portrait() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-[380px] [perspective:1000px]">
      <div aria-hidden="true" className="absolute -inset-10 rounded-full bg-[radial-gradient(circle,rgb(168_85_247/0.35),rgb(34_211_238/0.12)_45%,transparent_70%)] blur-2xl" />
      ...
      >
        <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-900">
          {failed ? (
            <div className="flex aspect-[4/5] items-center justify-center text-zinc-600">
            ...
            <img
              ...
              className="aspect-[4/5] w-full scale-[1.12] object-cover object-[50%_20%]"
              fetchPriority="high"
            />
          )}
          <motion.div aria-hidden="true" style={{ background: glare }} className="pointer-events-none absolute inset-0 mix-blend-overlay" />
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/70 to-transparent p-5 pt-16" style={{ transform: 'translateZ(40px)' }}>
            <p className="text-lg font-bold text-white">{profile.name}</p>
            <p className="text-sm text-zinc-300">{profile.title}</p>
          </div>
        </div>

        {/* Floating badges, pushed forward in 3D */}
        {bankClient?.parent && (
          <div style={{ transform: 'translateZ(60px)' }} className="absolute top-6 -left-6 sm:-left-12">
            <motion.div
              animate={{ y: [0, -8, 0] }}
              transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}
              className="flex items-center gap-2.5 rounded-2xl border border-white/10 bg-black/80 p-2 pr-3.5 shadow-[0_10px_40px_-10px_rgb(16_185_129/0.6)] backdrop-blur-xl"
            >
              ...
              <span className="text-left leading-tight">
                <span className="block text-[10px] tracking-wider text-emerald-300 uppercase">Client</span>
                <span className="block text-xs font-semibold text-white">BNP Paribas group</span>
              </span>
            </motion.div>
          </div>
        )}
        <div style={{ transform: 'translateZ(50px)' }} className="absolute -right-4 bottom-24 sm:-right-10">
          <motion.div
            ...
            className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/80 px-3.5 py-2.5 shadow-[0_10px_40px_-10px_rgb(168_85_247/0.7)] backdrop-blur-xl"
          >
            <ShieldCheck className="size-4 text-violet-300" aria-hidden="true" />
            <span className="text-xs font-semibold text-white">MFA · Security · QA</span>
          </motion.div>
        </div>
      </motion.div>
    </div>
