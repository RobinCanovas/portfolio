...
import { findExperience, projectFilters, projects } from '../data/profile';
    ...
    <Section id="projects" eyebrow="03 · Work" title="Projects">
          ...
          return (
            <button
              ...
              className={`group inline-flex shrink-0 items-center gap-1.5 rounded-full border px-4 py-1.5 font-mono text-xs transition ${
                on ? 'border-violet-400 bg-violet-500/20 text-white shadow-[0_0_16px_rgb(168_85_247/0.35)]' : 'border-white/10 text-zinc-400 hover:border-white/30 hover:text-white'
              }`}
            >
      ...
      <motion.ul layout className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <AnimatePresence mode="popLayout">
          {visible.map((p) => {
            const exp = p.experienceId ? findExperience(p.experienceId) : undefined;
            return (
              <motion.li
                key={p.id}
                id={`project-${p.id}`}
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span className="flex min-w-0 items-center gap-2">
                      ...
                      <span className="truncate font-mono text-[11px] text-cyan-300/80">{p.context}</span>
                    </span>
                    {p.confidential ? (
                      <Lock className="size-4 shrink-0 text-amber-300" aria-label="Confidential" />
                    ) : (
                  ...
                  <div className="mt-auto flex items-center justify-between pt-5 text-xs">
                    {p.demo ? (
                      <span className="inline-flex items-center gap-1.5 text-emerald-300">
                        <PlayCircle className="size-4" aria-hidden="true" /> Interactive demo
                      </span>
                    ) : (
                      <span className="text-zinc-500">{p.year}</span>
                    )}
                    <span className="inline-flex items-center gap-1 text-zinc-400 transition group-hover:text-white">
                      Case study <ArrowUpRight className="size-4 transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" aria-hidden="true" />
                    </span>
                  </div>
                </a>
              </motion.li>
